...
func defaultedType(conf *conf) *conf {
	if conf.flagName == "kubeconfig" {
		...
		case conf.flagName == "config", conf.flagName == "audit-policy-file":
		...
		for _, newConf := range k.confs {
			...
			if conf == nil {
				...
				if conf.flagType != newConf.flagType {
					panic("TODO: different types accross versions")
				}
				if conf.flagDefault != newConf.flagDefault {
					// panic(fmt.Errorf("TODO: different types accross versions: %s %s %q / %q",
					// 	k.name, conf.flagName, conf.flagDefault, newConf.flagDefault))
				}
			}
...
func printKomponentCode(komp *komponent) string {
	printAssignment := func(c *conf, v string) string {
		...
		case "bool":
			return fmt.Sprintf("res.%s, _ = strconv.ParseBool(%s)", toGoField(c.flagName), v)
		case "float64":
			return fmt.Sprintf("res.%s, _ = strconv.ParseFloat(%s, 64)", toGoField(c.flagName), v)
		case "int", "uint":
			return fmt.Sprintf("res.%s, _ = strconv.Atoi(%s)", toGoField(c.flagName), v)
		case "time.Duration":
			return fmt.Sprintf("res.%s, _ = time.ParseDuration(%s)", toGoField(c.flagName), v)
		case "[]string":
			return fmt.Sprintf("res.%s = strings.Split(%s, \",\")", toGoField(c.flagName), v)
		case "*K8sKubeconfigMeta":
			return fmt.Sprintf("res.%s = loadKubeconfigMeta(%s)", toGoField(c.flagName), v)
		case "*K8sCertFileMeta":
			return fmt.Sprintf("res.%s = loadCertFileMeta(%s)", toGoField(c.flagName), v)
		case "*K8sKeyFileMeta":
			return fmt.Sprintf("res.%s = loadKeyFileMeta(%s)", toGoField(c.flagName), v)
		case "*K8sTokenFileMeta":
			return fmt.Sprintf("res.%s = loadTokenFileMeta(%s)", toGoField(c.flagName), v)
		case "*K8sConfigFileMeta":
			return fmt.Sprintf("res.%s = loadConfigFileMeta(%s)", toGoField(c.flagName), v)
		case "*K8sEncryptionProviderConfigFileMeta":
			return fmt.Sprintf("res.%s = loadEncryptionProviderConfigFileMeta(%s)", toGoField(c.flagName), v)
		case "*K8sDirMeta":
			return fmt.Sprintf("res.%s = loadDirMeta(%s)", toGoField(c.flagName), v)
		default:
	...
	s += "}\n"
	s += fmt.Sprintf("func newK8s%sConfig(flags map[string]string) *K8s%sConfig {\n", goStructName, goStructName)
...
func toGoJSONTag(s string) string {
	s = strings.ReplaceAll(strings.Title(s), "-", "")
...
func scanEtcdHelpLine(line string) (*conf, bool) {
	...
	conf.flagDefault, str, ok = eatRegexp(str, "'(\\S*)'", 1)
	if ok {
...
func isCISFlag(flag string) bool {
	for _, cisFlag := range cisFlags {
		if "--"+flag == cisFlag {
			return true
		}
	}
