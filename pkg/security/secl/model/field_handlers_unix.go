...
func (ev *Event) resolveFields(forADs bool) {
	...
	if ev.ProcessContext.Process.IsNotKworker() {
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.ProcessContext.Process.FileEvent)
	}
	if ev.ProcessContext.Process.IsNotKworker() {
	...
	if ev.ProcessContext.Process.HasInterpreter() {
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.ProcessContext.Process.LinuxBinprm.FileEvent)
	}
	if ev.ProcessContext.Process.HasInterpreter() {
	...
	if ev.ProcessContext.HasParent() && ev.ProcessContext.Parent.IsNotKworker() {
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.ProcessContext.Parent.FileEvent)
	}
	if ev.ProcessContext.HasParent() && ev.ProcessContext.Parent.IsNotKworker() {
	...
	if ev.ProcessContext.HasParent() && ev.ProcessContext.Parent.HasInterpreter() {
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.ProcessContext.Parent.LinuxBinprm.FileEvent)
	}
	if ev.ProcessContext.HasParent() && ev.ProcessContext.Parent.HasInterpreter() {
	...
	case "chmod":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Chmod.File)
	case "chown":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Chown.File)
	...
	case "exec":
		...
		if ev.Exec.Process.IsNotKworker() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Exec.Process.FileEvent)
		}
		...
		if ev.Exec.Process.HasInterpreter() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Exec.Process.LinuxBinprm.FileEvent)
		}
	...
	case "exit":
		...
		if ev.Exit.Process.IsNotKworker() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Exit.Process.FileEvent)
		}
		...
		if ev.Exit.Process.HasInterpreter() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Exit.Process.LinuxBinprm.FileEvent)
		}
	...
	case "link":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Link.Source)
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Link.Target)
	case "load_module":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.LoadModule.File)
	...
	case "mkdir":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Mkdir.File)
	case "mmap":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.MMap.File)
	case "mount":
	...
	case "open":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Open.File)
	case "ptrace":
		...
		if ev.PTrace.Tracee.Process.IsNotKworker() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.PTrace.Tracee.Process.FileEvent)
		}
		...
		if ev.PTrace.Tracee.Process.HasInterpreter() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.PTrace.Tracee.Process.LinuxBinprm.FileEvent)
		}
		...
		if ev.PTrace.Tracee.HasParent() && ev.PTrace.Tracee.Parent.IsNotKworker() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.PTrace.Tracee.Parent.FileEvent)
		}
		...
		if ev.PTrace.Tracee.HasParent() && ev.PTrace.Tracee.Parent.HasInterpreter() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.PTrace.Tracee.Parent.LinuxBinprm.FileEvent)
		}
	...
	case "removexattr":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.RemoveXAttr.File)
	...
	case "rename":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Rename.Old)
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Rename.New)
	case "rmdir":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Rmdir.File)
	case "selinux":
	...
	case "setxattr":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.SetXAttr.File)
	...
	case "signal":
		...
		if ev.Signal.Target.Process.IsNotKworker() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Signal.Target.Process.FileEvent)
		}
		...
		if ev.Signal.Target.Process.HasInterpreter() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Signal.Target.Process.LinuxBinprm.FileEvent)
		}
		...
		if ev.Signal.Target.HasParent() && ev.Signal.Target.Parent.IsNotKworker() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Signal.Target.Parent.FileEvent)
		}
		...
		if ev.Signal.Target.HasParent() && ev.Signal.Target.Parent.HasInterpreter() {
			_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Signal.Target.Parent.LinuxBinprm.FileEvent)
		}
	...
	case "splice":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Splice.File)
	case "unlink":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Unlink.File)
	case "unload_module":
	case "utimes":
		...
		_ = ev.FieldHandlers.ResolveHashes(ev, &ev.Utimes.File)
	}
}
...
type FieldHandlers interface {
	...
	ResolveHashes(ev *Event, e *FileEvent) []string
...
func (dfh *DefaultFieldHandlers) ResolveHashes(ev *Event, e *FileEvent) []string { return e.Hashes }
