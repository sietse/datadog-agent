...
func TestResolver_ComputeHashes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resolver := NewResolver(tt.config, client)
			got := resolver.ComputeHashes(tt.args.event, tt.args.file)
	...
	for _, bb := range benchmarks {
		for _, fc := range bb.fileSizes {
			b.Run(bb.name+"/"+fc.name, func(caseB *testing.B) {
				...
				resolver := NewResolver(bb.config, client)

				caseB.ResetTimer()

				for i := 0; i < caseB.N; i++ {
					got := resolver.ComputeHashes(&model.Event{
						FieldHandlers: &model.DefaultFieldHandlers{},
						Type:          uint32(model.ExecEventType),
						ProcessContext: &model.ProcessContext{
							Process: model.Process{
